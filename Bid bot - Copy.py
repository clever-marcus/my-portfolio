...
class Steem_node():
    def __init__(self, block, account):
        ...
        self.nodes = ['https://rpc.steemviz.com', 'https://rpc.buildteam.io', 'https://api.steemit.com']
        ...
        self.queue = dataclasses.Queue(self.steem, self.account)
        print('\nConnected to: {}'. format(self.nodes[0]))

    def process_timestamp(self, block):
        timestamp = block['timestamp']
        datetime_object = datetime.strftime(timestamp, '%Y-%m-%dT%H:%M:%S')
    ...
    def run(self):
        while True:
            try:
                stream = self.b.stream_from(start_block=self.block,full_blocks=True)
                for block in stream:
